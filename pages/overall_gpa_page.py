...
class OverallGpaPage(QDialog, form_class):
    def __init__(self, parent_geometry, main_window):
        ...
        self.copyButtonA.clicked.connect(lambda: self.copy_to_clipboard(self.format_copy_current()))
        self.copyButtonB.clicked.connect(lambda: self.copy_to_clipboard(self.format_copy_if()))
    ...
    def calculate_gpa(self):
        ...
        self.textBrowser6.setText(f"{gpa2:.2f}")
    ...
    def format_copy_current(self):
        ...
        self.copy_text_to_clipboard(copy_text)
    ...
    def format_copy_if(self):
        ...
        pt_values = [v if v else "0" for v in pt_values]  # 빈 값은 0 처리
    ...
    def calculate_target_gpa(self):
        ...
        try:
            ...
            self.show_error("올바른 숫자를 입력하세요.")
            return

        if A is None or B is None:
            self.show_error("GPA 데이터가 부족합니다.")
            return
    ...
    def convert_gpa(self):
        ...
        for browser in [self.textBrowser1, self.textBrowser2, self.textBrowser5, self.textBrowser6]:
            browser.setPlainText(convert(browser.toPlainText()))
